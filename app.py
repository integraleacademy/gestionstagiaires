...
def _cnaps_unknown_training_type_is_allowed(training_type: Optional[str]) -> bool:
...
def _collect_cnaps_unknown_trainees(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for session_data in data.get("sessions", []):
        ...
        if not _cnaps_unknown_training_type_is_allowed(training_type):
            continue
...
def _find_cnaps_trainee_match(
    ...
    # Fallback demandé métier: recherche uniquement par NOM.
    # Si plusieurs stagiaires partagent le même nom, on préfère quand même
    # celui qui matche le prénom si l'information existe, sinon le premier.
    by_last_name = trainees_by_last_name.get(last_name) or []
    if not by_last_name:
        return None

    normalized_first = _normalize_person_name(first_name)
    if normalized_first:
        for entry in by_last_name:
    ...
    for s in data.get("sessions", []):
        ...
        for t in _session_trainees_list(s):
            ...
            if _is_truthy(t.get("scotia_hidden")):
                included_reason = f"{included_reason}_despite_scotia_hidden"
...
def _notify_scotia_complementary_documents(t: Dict[str, Any], s: Dict[str, Any], file_count: int, source_label: str) -> bool:
    trainee_display_name = _format_trainee_name(t.get("first_name", ""), t.get("last_name", ""))
    session_name = _session_get(s, "name", "") or str(s.get("id") or "")
    today_fr = fr_date(datetime.datetime.utcnow().strftime("%Y-%m-%d"))
    comment = (t.get("scotia_comment") or "").strip()
    comment_html = f"<p><strong>Commentaire SCOTIA :</strong><br>{html.escape(comment).replace(chr(10), '<br>')}</p>" if comment else ""
    subject = "Documents complémentaires déposés (VAE)"
    html_body = mail_layout(f"""
    <h2 style="margin:0 0 12px 0;color:#0f172a;">📎 Documents complémentaires déposés</h2>
    <p>Des documents complémentaires viennent d'être déposés depuis {html.escape(source_label)}.</p>
    <p><strong>Stagiaire :</strong> {html.escape(trainee_display_name or "—")}</p>
    <p><strong>Email :</strong> {html.escape((t.get("email") or "").strip() or "—")}</p>
    <p><strong>Téléphone :</strong> {html.escape((t.get("phone") or "").strip() or "—")}</p>
    <p><strong>Session :</strong> {html.escape(session_name or "—")}</p>
    <p><strong>Nombre de fichier(s) :</strong> {file_count}</p>
...
def _append_complementary_documents(session_id: str, trainee_id: str, t: Dict[str, Any], incoming_files: List[Any], storage_area: str) -> int:
    target = _ensure_complementary_documents_entry(t)
    accepted_exts = {".pdf", ".jpg", ".jpeg", ".png", ".webp"}
...
def scotia_upload_complementary_documents(session_id: str, trainee_id: str):
    ...
    try:
        stored_count = _append_complementary_documents(session_id, trainee_id, t, files, 'public_documents')
    except Exception:
        stored_count = 0
    if stored_count:
        s['trainees'] = trainees
...
def _etiquette_template_name(training_type: str) -> Optional[str]:
    template_map = {
        ...
        "DIRIGEANT": "etiquette_dirigeant_initial.docx",
...
def admin_etiquette_print_page(session_id: str, trainee_id: str):
    ...
    formation_label_text = formation_map.get(training_type, f"FORMATION {training_type}".strip())
    if "DIRIGEANT" in training_type and "VAE" in training_type:
        formation_label_text = "FORMATION DESP VAE"
...
VAE_STATUS_STEPS = {
    ...
    "financement_l2_validated": {"label": "Financement L2 validé", "pill": "green"},
...
VAE_RELANCE_CONFIGS = {
    ...
    "rdv_recevabilite": {
        ...
        "blocked_if_vae_status_in": ["financement_validated", "livret_2_todo", "livret_2_analysis", "livret_2_validated", "financement_l2_validated", "jury", "certified"],
    ...
    "livret_2": {
        ...
        "blocked_if_vae_status_in": ["livret_2_analysis", "livret_2_validated", "financement_l2_validated", "jury", "certified"],
...
APS_CONVOCATION_CENTER_NAME = "Intégrale Academy"
APS_CONVOCATION_CENTER_ADDRESS = "54 chemin du Carreou"
...
def _safe_filename_part(value: Any) -> str:
    ...
    return normalized or "stagiaire"


def _format_long_fr_date(value: str) -> str:
    value = (value or "").strip()
    if not value:
        return ""
    months = ["", "janvier", "février", "mars", "avril", "mai", "juin", "juillet", "août", "septembre", "octobre", "novembre", "décembre"]
    try:
        dt = datetime.datetime.strptime(value[:10], "%Y-%m-%d").date()
...
def _docx_replace_text(doc: Document, replacements: Dict[str, str]) -> None:
    """Replace APS template tokens while preserving the Word document structure.

    YPAREO exports often split placeholders across several runs.  We therefore
    rebuild only the paragraph text containing placeholders and leave all other
    paragraphs, tables, images, headers and footers untouched for LibreOffice to
    convert from the original Word layout.
    """
...
def _find_unresolved_docx_variables(doc: Document) -> List[str]:
    """Return remaining YPAREO/Word placeholders that would leak into the PDF."""
    found: Set[str] = set()
...
LIBREOFFICE_MISSING_MESSAGE = (
    "LibreOffice est introuvable sur le serveur : impossible de convertir le "
    "modèle Word APS en PDF. Aucun PDF simplifié de secours n’a été envoyé."
)
...
def _check_libreoffice_available() -> None:
    if not _find_libreoffice_binary():
...
def _build_aps_convocation_context(session_obj: Dict[str, Any], trainee: Dict[str, Any]) -> Dict[str, str]:
    ...
    trainee_address = str(trainee.get("address") or trainee.get("adresse") or "").strip()
    trainee_zip_code = str(trainee.get("zip_code") or trainee.get("code_postal") or trainee.get("postal_code") or "").strip()
    trainee_city = str(trainee.get("city") or trainee.get("ville") or "").strip()
    return {"civility": str(trainee.get("civility") or trainee.get("civilite") or "").strip(), "first_name": str(trainee.get("first_name") or trainee.get("prenom") or "").strip(), "last_name": str(trainee.get("last_name") or trainee.get("nom") or "").strip().upper(), "training_name": str(training_name).strip(), "date_start": str(_session_get(session_obj, "date_start", "")).strip(), "date_end": str(_session_get(session_obj, "date_end", "")).strip(), "aps_remote_start": str(_session_get(session_obj, "aps_remote_start", "") or _session_get(session_obj, "date_start", "")).strip(), "aps_remote_end": str(_session_get(session_obj, "aps_remote_end", "") or _session_get(session_obj, "date_start", "")).strip(), "aps_in_person_start": aps_in_person_start, "aps_in_person_end": aps_in_person_end, "exam_date": str(_session_get(session_obj, "exam_date", "")).strip(), "exam_time": str(_session_get(session_obj, "exam_time", "") or session_obj.get("heure_examen") or "08h00").strip() or "08h00", "convocation_time": str(_session_get(session_obj, "convocation_time", "") or session_obj.get("heure_convocation") or "08h30").strip() or "08h30", "center_name": APS_CONVOCATION_CENTER_NAME, "center_address": APS_CONVOCATION_CENTER_ADDRESS, "center_zip_code": APS_CONVOCATION_CENTER_ZIP, "center_city": APS_CONVOCATION_CENTER_CITY, "address": trainee_address, "zip_code": trainee_zip_code, "city": trainee_city, "today": datetime.datetime.utcnow().strftime("%d/%m/%Y")}
...
def generate_convocation_aps_pdf(session_obj: Dict[str, Any], trainee: Dict[str, Any], output_path: str) -> str:
    """Disabled legacy generator: APS convocations must come from the Word template."""
    raise RuntimeError(
        "Génération PDF APS simplifiée désactivée : utilisez templates_word/convocationaps.docx."
    )
...
def _generate_aps_convocation_from_docx_template(session_obj: Dict[str, Any], trainee: Dict[str, Any], session_id: str = "", trainee_id: str = "") -> Tuple[str, str]:
    ...
    place = f"{ctx['center_name']} ({ctx['center_address']} - {ctx['center_zip_code']} {ctx['center_city']})"
    ...
    replacements = {
        ...
        "[DateConvocation]": fr_date(ctx["aps_in_person_start"]), "[heureConvocation]": ctx["convocation_time"], "[=TODAY()]": ctx["today"], "{{date_du_jour}}": ctx["today"], "{{duree_totale}}": "175 h", "{{duree_elearning}}": "62 h", "{{duree_presentiel}}": "113 h",
...
def _generate_aps_convocation_pdf(session_obj: Dict[str, Any], trainee: Dict[str, Any], session_id: str = "", trainee_id: str = "") -> str:
    """Generate the APS PDF exclusively from templates_word/convocationaps.docx."""
    pdf_path, _docx_path = _generate_aps_convocation_from_docx_template(session_obj, trainee, session_id, trainee_id)
    return pdf_path
...
@admin_login_required
@admin_write_required
def admin_send_aps_convocation(session_id: str, trainee_id: str):
    data = load_data()
    s, trainees, t = _find_session_trainee(data, session_id, trainee_id)
    if not s or not t:
...
@admin_login_required
def admin_view_aps_convocation(session_id: str, trainee_id: str):
    data = load_data()
...
def api_cnaps_import_pre():
    ...
    for sess in data.get("sessions", []):
        ...
        for trainee in trainees:
            ...
            trainees_index.setdefault(key, []).append(entry)
            trainees_by_last_name.setdefault(key[0], []).append(entry)
