...
class DigiformaAttendanceTests(unittest.TestCase):
    ...
    def test_complete_rebuilt_attendance_keeps_all_courses_and_requested_metrics(self):
        result, metadata = prepare_digiforma_attendance(attendance_pdf(), self.signature)
        reader = PdfReader(io.BytesIO(result))
        text = '\n'.join(page.extract_text() for page in reader.pages)
        self.assertEqual(metadata['processing_version'], 3)
        ...
        self.assertGreaterEqual(len(list(reader.pages[-1].images)), 2)
    ...
    def test_long_module_text_reflows_and_all_glyphs_stay_inside_page_margins(self):
        result, _ = prepare_digiforma_attendance(attendance_pdf(long_label=True), self.signature)
    ...
    def test_every_connection_survives_pagination_and_headers_repeat(self):
        result, _ = prepare_digiforma_attendance(attendance_pdf(connection_count=70), self.signature)
        reader = PdfReader(io.BytesIO(result))
        ...
        self.assertIn('Durée totale du journal', texts[-1])
    ...
    def test_missing_module_is_rejected_instead_of_silently_omitted(self):
        ...
        with self.assertRaisesRegex(ValueError, 'modules Digiforma'):
            prepare_digiforma_attendance(malformed, self.signature)
