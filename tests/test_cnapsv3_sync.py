...
class CnapsTrackingTests(unittest.TestCase):
    ...
    def test_cnaps_public_annuaire_notifies_when_unknown_becomes_active(self):
        ...
        self.assertEqual(data["cnaps_status_change_notifications"]["DOE|1234567"].get("seen_at"), "")

    def test_cnaps_status_change_badge_counts_and_marks_unseen_notifications(self):
        data = {"cnaps_status_change_notifications": {
            "DOE|1234567": {"signature": "AP SH • ACTIF", "seen_at": ""},
            "SMITH|7654321": {"signature": "CP SH • ACTIF", "seen_at": "2026-07-15T10:00:00Z"},
        }}

        self.assertEqual(gestion_app._cnaps_status_change_unread_count(data), 1)
        self.assertTrue(gestion_app._mark_cnaps_status_change_notifications_seen(data))
        self.assertEqual(gestion_app._cnaps_status_change_unread_count(data), 0)
        self.assertTrue(data["cnaps_status_change_notifications"]["DOE|1234567"].get("seen_at"))
