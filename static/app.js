...
(function(){
  ...
  function showTraineeSummary(payload){
    ...
    const person=payload.trainee||{},progress=payload.progress||{},schedule=payload.schedule||{},trainingName=person.training_type||person.session_name||'Formation';
    const scheduleLines=schedule.hybrid
      ? [["Distanciel",schedule.remote],["Présentiel",schedule.in_person],["Examen",schedule.exam?`le ${schedule.exam}`:'']]
      : [["",schedule.formation?`${trainingName} · ${schedule.formation.charAt(0).toUpperCase()+schedule.formation.slice(1)}`:trainingName],["Examen",schedule.exam?`le ${schedule.exam}`:'']];
    const scheduleHtml=scheduleLines.filter(([,value])=>value).map(([label,value])=>`<p>${label?`<strong>${escapeHtml(label)} :</strong> `:''}${escapeHtml(value)}</p>`).join('');
    modal.innerHTML=`<div class="trainee-summary__backdrop" data-summary-close></div><section class="trainee-summary__card" role="dialog" aria-modal="true" aria-labelledby="traineeSummaryTitle"><header><div><span class="trainee-summary__kicker">Vue à 360°</span><h2 id="traineeSummaryTitle">${escapeHtml(`${person.first_name||''} ${person.last_name||''}`)}</h2><div class="trainee-summary__schedule">${scheduleHtml}</div></div><button type="button" data-summary-close aria-label="Fermer">×</button></header><div class="trainee-summary__score"><div class="trainee-summary__ring" style="--progress:${Number(progress.percent)||0}"><strong>${Number(progress.percent)||0}%</strong></div><div><strong>${progress.completed||0} étapes validées sur ${progress.total||0}</strong><span>${progress.percent===100?'Le dossier est prêt.':'Les points restants sont visibles ci-dessous.'}</span></div></div><div class="trainee-summary__grid">${(payload.statuses||[]).map(item=>`<article class="is-${escapeHtml(item.state)}"><span class="trainee-summary__check">${item.state==='complete'?'✓':item.state==='neutral'?'—':'!'}</span><div><strong>${escapeHtml(item.label)}</strong><small>${escapeHtml(item.detail)}</small></div></article>`).join('')}</div><footer><a href="${escapeHtml(payload.public_url)}" target="_blank" rel="noopener">Ouvrir l’espace stagiaire</a><a class="is-primary" href="${escapeHtml(payload.admin_url)}">Ouvrir la fiche</a></footer></section>`;
