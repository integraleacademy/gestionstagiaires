...
(function(){
  ...
  function ensure(){let r=document.getElementById('appModalRoot');if(r)return r;r=document.createElement('div');r.id='appModalRoot';r.innerHTML=`<div class="app-modal-backdrop" role="presentation"><section class="app-modal-card" role="dialog" aria-modal="true" aria-labelledby="appModalTitle"><button type="button" class="app-modal-x" aria-label="Fermer">×</button><div class="app-modal-content"><div class="app-modal-icon" aria-hidden="true"></div><div class="app-modal-copy"><div class="app-modal-kicker"></div><h2 id="appModalTitle"></h2><p class="app-modal-message"></p><div class="app-modal-extra"></div></div></div><div class="app-modal-actions"><button type="button" class="app-modal-btn app-modal-secondary"></button><button type="button" class="app-modal-btn app-modal-primary"></button></div></section></div>`;document.body.appendChild(r);return r;}
  function show(opts={}){return new Promise(resolve=>{const tone=opts.type||'info',meta=tones[tone]||tones.info,root=ensure(),bd=root.firstElementChild,card=bd.querySelector('.app-modal-card'),x=bd.querySelector('.app-modal-x'),primary=bd.querySelector('.app-modal-primary'),secondary=bd.querySelector('.app-modal-secondary');card.className=`app-modal-card app-modal-${tone}`;bd.querySelector('.app-modal-icon').textContent=opts.icon||meta.icon;bd.querySelector('.app-modal-kicker').textContent=opts.kicker||meta.k;bd.querySelector('h2').textContent=opts.title||'Information';bd.querySelector('.app-modal-message').textContent=opts.message||'';bd.querySelector('.app-modal-extra').innerHTML=opts.html||'';primary.textContent=opts.primaryText||'Fermer';secondary.textContent=opts.secondaryText||'Annuler';secondary.style.display=opts.showSecondary?'':'none';x.style.display=opts.closable===false?'none':'';function done(v){bd.classList.remove('is-visible');document.body.classList.remove('app-modal-open');setTimeout(()=>{primary.onclick=secondary.onclick=x.onclick=bd.onclick=null;resolve(v);},120)}primary.onclick=()=>done(true);secondary.onclick=()=>done(false);x.onclick=()=>done(false);bd.onclick=e=>{if(e.target===bd&&opts.closable!==false)done(false)};document.onkeydown=e=>{if(e.key==='Escape'&&opts.closable!==false)done(false)};requestAnimationFrame(()=>{bd.classList.add('is-visible');document.body.classList.add('app-modal-open');primary.focus();});});}
  window.AppModal={show,alert:(message,options={})=>show({message,type:options.type||'info',title:options.title||((options.type==='error'||options.type==='danger')?'Action impossible':'Information'),primaryText:options.primaryText||'Fermer'}),confirm:(options={})=>show({type:options.type||'warning',title:options.title||'Confirmer l’action',message:options.message||'',primaryText:options.primaryText||'Confirmer',secondaryText:options.secondaryText||'Annuler',showSecondary:true,closable:true})};
